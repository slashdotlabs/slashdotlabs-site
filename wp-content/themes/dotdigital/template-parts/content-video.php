...
?>
	<article id="post-<?php the_ID(); ?>" <?php post_class('format-small-image '. $show_readmore .''); ?>>
		<div class="side-item content-padding big-padding with_background overflow-hidden">
			<div class="row">
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class('vertical-item content-padding big-padding with_background overflow-hidden '. $show_readmore . ''); ?>>
	<?php
			...
			<header class="entry-header">
				<?php if ( $post_categories == 'yes' ) : ?>
					<?php if ( in_array( 'category', get_object_taxonomies( get_post_type() ) ) && dotdigital_categorized_blog() ) : ?>
						<div
							class="categories-links theme_buttons color color1 small_height"><?php echo get_the_category_list( _x( ' ', 'Used between list items, there is a space after the comma.', 'dotdigital' ) ); ?></div>
					<?php endif; ?>
				<?php endif; ?>
				<!-- .item cats -->
				<?php
