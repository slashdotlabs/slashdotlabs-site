...
if ( is_singular() ) :
    ...
    <!-- tc: content-event -->
    <article
            id="post-<?php the_ID(); ?>" <?php post_class( 'vertical-item content-padding with_border   big-padding   overflow-hidden' ); ?>>
            ...
            <header class="entry-header content-start">
				...
				<?php if ( $post_categories == 'yes' ) : ?>
					<?php if ( in_array( 'category', get_object_taxonomies( get_post_type() ) ) && dotdigital_categorized_blog() ) : ?>
                        <div
                                class="categories-links darklinks"><?php echo get_the_category_list( _x( ' ', 'Used between list items, there is a space after the comma.', 'dotdigital' ) ); ?></div>
					<?php endif; ?>
				<?php endif; ?>
                <!-- .item cats -->
...
//blog feed layout
	...
	?>
    <article
            id="post-<?php the_ID(); ?>" <?php post_class( 'vertical-item content-padding with_background overflow-hidden' ); ?>>
        <div class="side-item">
            <div class="row">
                <div class="col-md-4">
