...
<div class="form-group comment-form-author">' . '<label for="author">' . esc_html__( 'Your Name', 'dotdigital' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label>' .
			                   '<input id="author" class="form-control" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . $html_req . ' placeholder="' . esc_html__( '', 'dotdigital' ) . '"></div>
			                   </div>',
...
<div class="form-group comment-form-email"><label for="email">' . esc_html__( 'Email Address', 'dotdigital' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label>' .
			                   '<input id="email" class="form-control" name="email" ' . ( $html5 ? 'type="email"' : 'type="text"' ) . ' value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30" ' . $aria_req . $html_req . ' placeholder="' . esc_html__( '', 'dotdigital' ) . '"/></div>
			                   </div>',
...
<div class="form-group comment-form-phone">' . '<label for="phone">' . esc_html__( 'Phone Number', 'dotdigital' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label>' .
			                  '<input id="phone" class="form-control" name="phone" type="text"  size="30"' . $aria_req . $html_req . ' placeholder="' . esc_html__( '', 'dotdigital' ) . '"></div>
			                   </div>',
			'comment_field' => '<div class="col-xs-12"><div class="form-group comment-form-comment"><label for="comment">' . esc_html_x( 'Your Comment', 'noun', 'dotdigital' ) . '</label><textarea id="comment"  class="form-control" name="comment" cols="45" rows="5"  aria-required="true" required="required"  placeholder="' . esc_html__( '', 'dotdigital' ) . '"></textarea></div></div>',
		);
...
if ( ! function_exists( 'dotdigital_shortcode_atts_gallery_trigger' ) ) :
	function dotdigital_shortcode_atts_gallery_trigger( $add_filter = true ) {
		if ( $add_filter ) {
			...
			remove_filter( 'shortcode_atts_gallery', 'dotdigital_filter_fw_shortcode_atts_gallery', 10, 3 );
		}
...
if ( !function_exists( 'dotdigital_action_widgets_init' ) ) :
	function dotdigital_action_widgets_init() {
		...
		register_sidebar( array(
			'name'          => esc_html__( 'Footer Column', 'dotdigital' ),
			'id'            => 'sidebar-footer-1',
...
if ( ! function_exists( 'dotdigital_filter_theme_fw_ext_backups_demos' ) ) :
	...
	function dotdigital_filter_theme_fw_ext_backups_demos( $demos ) {
		if ( class_exists( 'FW_Ext_Backups_Demo' ) ) :
			$demos_array = array(
				'dotdigital-demo' => array(
					'title'        => esc_html__( 'Dotdigital Demo', 'dotdigital' ),
					'screenshot'   => 'http://webdesign-finder.com/dotdigital/demo/screenshot.png',
					'preview_link' => 'http://webdesign-finder.com/dotdigital/',
				),
			);

			$download_url = 'http://webdesign-finder.com/dotdigital/demo/';
